...
;* Date/Time created: Wed Feb 12 23:18:04 2025                                *
;******************************************************************************
...
;	optpru /tmp/TI4jLGp6yuk /tmp/TI4jLey4ANu 
;	acpiapru -@/tmp/TI4jLJEsH0f 
	.sect	".text:uartPutC"
	.clink
	.global	||uartPutC||
;----------------------------------------------------------------------
...
;----------------------------------------------------------------------
        LDI32     r1, 0x48024014        ; [ALU_PRU] |127| 
;* --------------------------------------------------------------------------*
;*   BEGIN LOOP ||$C$L1||
;*
...
;----------------------------------------------------------------------
        ...
        LDI32     r1, 0x48024000        ; [ALU_PRU] |128| 
...
;     | smitted                                                                
;----------------------------------------------------------------------
        LDI32     r1, 0x48024014        ; [ALU_PRU] |135| 
;* --------------------------------------------------------------------------*
;*   BEGIN LOOP ||$C$L4||
;*
...
;----------------------------------------------------------------------
        ...
        LDI32     r1, 0x4802400c        ; [ALU_PRU] |47| 
        MOV       r3.w0, r3.w2          ; [ALU_PRU] 
...
; 106 | if (UART_LSR & 0x1) {                                                  
;----------------------------------------------------------------------
        LDI32     r0, 0x48024000        ; [ALU_PRU] |106| $O$C1
        MOV       r3.w0, r3.w2          ; [ALU_PRU] 
...
;----------------------------------------------------------------------
        LDI32     r18, 0x48024000       ; [ALU_PRU] |61| $O$C6
;----------------------------------------------------------------------
...
;* --------------------------------------------------------------------------*
        MOV       r16.w2, r3.w2         ; [ALU_PRU] 
;----------------------------------------------------------------------
;  79 | int16_t receiveRemainingMessage(uint8_t* buf) {                        
;----------------------------------------------------------------------
        MOV       r15, r14              ; [ALU_PRU] |79| buf
;----------------------------------------------------------------------
;  80 | uint16_t i = 0;                                                        
;  81 | while (i < MAX_PAYLOAD_LEN) {                                          
;----------------------------------------------------------------------
        LDI       r16.w0, 0x0000        ; [ALU_PRU] |80| i
;* --------------------------------------------------------------------------*
;*   BEGIN LOOP ||$C$L12||
;*
;*   Loop source line                : 81
...
;----------------------------------------------------------------------
        ADD       r14, r15, r16.w0      ; [ALU_PRU] |82| buf,i
        JAL       r3.w2, ||uartGetC||   ; [ALU_PRU] |82| uartGetC
        QBEQ      ||$C$L13||, r14.b0, 0x00 ; [ALU_PRU] |82| 
;* --------------------------------------------------------------------------*
;----------------------------------------------------------------------
;  83 | i++;                                                                   
...
;----------------------------------------------------------------------
        ADD       r16.w0, r16.w0, 0x01  ; [ALU_PRU] |83| i,i
;* --------------------------------------------------------------------------*
||$C$L13||:    
        ZERO      &r0, 4                ; [ALU_PRU] |36| i
;* --------------------------------------------------------------------------*
;*   BEGIN LOOP ||$C$L14||
;*
;*   Loop source line                : 36
...
;* --------------------------------------------------------------------------*
||$C$L14||:    
        LDI32     r1, 0x48024014        ; [ALU_PRU] |38| 
        LBBO      &r1, r1, 0, 4         ; [ALU_PRU] |38| 
        QBBS      ||$C$L15||, r1, 0x00  ; [ALU_PRU] |38| 
;* --------------------------------------------------------------------------*
        .newblock
        LDI32    r1, 5199
$1:     SUB      r1, r1, 1
        QBNE     $1, r1, 0             ; [ALU_PRU] |42| 
        ADD       r0, r0, 0x01          ; [ALU_PRU] |36| i,i
        LDI32     r14, 0x80000037       ; [ALU_PRU] |36| 
        MOV       r1, r0                ; [ALU_PRU] |36| i
        XOR       r1.b3, r1.b3, 0x80    ; [ALU_PRU] |36| 
        QBLT      ||$C$L14||, r14, r1   ; [ALU_PRU] |36| 
;* --------------------------------------------------------------------------*
        JMP       ||$C$L16||            ; [ALU_PRU] |36| 
;* --------------------------------------------------------------------------*
||$C$L15||:    
        LDI32     r0, 0x80000000        ; [ALU_PRU] |81| 
        XOR       r1, r16.w0, r0        ; [ALU_PRU] |81| i
        LDI32     r0, 0x800000ff        ; [ALU_PRU] |81| 
        QBLT      ||$C$L12||, r0, r1    ; [ALU_PRU] |81| 
;* --------------------------------------------------------------------------*
||$C$L16||:    
;----------------------------------------------------------------------
;  89 | return i; // Return the number of bytes received                       
;----------------------------------------------------------------------
        MOV       r14.w0, r16.w0        ; [ALU_PRU] |89| i
        JMP       r16.w2                ; [ALU_PRU] 
...
||$C$L17||:    
        ...
        LDI32     r1, 0x4804c138        ; [ALU_PRU] |33| 
...
;----------------------------------------------------------------------
        ...
        LDI       r17, 0x0012           ; [ALU_PRU] |68| 
        LDI       r18, 0x0013           ; [ALU_PRU] |68| 
...
;----------------------------------------------------------------------
;  71 | void main() {                                                          
;  72 | struct pru_rpmsg_transport transport;                                  
;----------------------------------------------------------------------
...
;  75 | uint16_t len = 0;                                                      
;----------------------------------------------------------------------
        LDI       r0.w0, 0x00           ; [ALU_PRU] |73| 
        SBBO      &r3.b2, r2, 66, 7     ; [ALU_PRU] 
;----------------------------------------------------------------------
;  77 | pruInit(&transport);                                                   
;----------------------------------------------------------------------
        ADD       r14, r2, 0            ; [ALU_PRU] |77| transport,transport
        SBBO      &r0.w0, r2, 60, 2     ; [ALU_PRU] |73| src
        SBBO      &r0.w0, r2, 62, 2     ; [ALU_PRU] |74| dst
        SBBO      &r0.w0, r2, 64, 2     ; [ALU_PRU] |75| len
;----------------------------------------------------------------------
;  78 | // Need to initialize src and dst                                      
;----------------------------------------------------------------------
        JAL       r3.w2, ||pruInit||    ; [ALU_PRU] |77| pruInit
;* --------------------------------------------------------------------------*
;*   BEGIN LOOP ||$C$L21||
;*
;*   Loop source line                : 79
...
||$C$L21||:    
;----------------------------------------------------------------------
;  79 | while(pru_rpmsg_receive(&transport, &src, &dst, transmitBuf, &len) != P
;     | RU_RPMSG_SUCCESS);                                                     
;  80 | memset(transmitBuf, 0, RPMSG_MESSAGE_SIZE);                            
;  82 | while (1) {                                                            
;  83 |     // Is there a message to transmit?                                 
;  84 |     if (transmitBuf[0] != 0 || pru_rpmsg_receive(&transport, &src, &dst
;     | , transmitBuf, &len) == PRU_RPMSG_SUCCESS) {                           
;  85 |         if (isBusIdle(CHECKS_TILL_BUS_IDLE)) {                         
;  86 |             // Send MID. Using uartWrite over uartPutC so that it waits
;     |  to                                                                    
;  87 |             // return until byte is transmitted.                       
;  88 |             uartWrite(transmitBuf, 1);                                 
;  89 |             // Arbitration: Send MID. If we recv anything and our MID i
;     | s                                                                      
;  90 |             // greater then we lose arbitration. Otherwise continue    
;  91 |             // writing the message.                                    
;  92 |             if (uartGetC(&receiveBuf[0]) && transmitBuf[0] > receiveBuf
;     | [0]) {                                                                 
;  93 |                 // We lost arbitration so read the remaining message.  
;  94 |                 uint16_t recvLen = receiveRemainingMessage(&receiveBuf[
;     | 1]);                                                                   
;  95 |                 pru_rpmsg_send(&transport, dst, src, receiveBuf, recvLe
;     | n+1);                                                                  
;  96 |             } else {                                                   
;  97 |                 // Either no one else is talking or we won arbitration 
;  98 |                 // uartWrite will only return once all bytes have been
;     | sent.                                                                  
;  99 |                 uartWrite(transmitBuf + 1, len - 1);                   
; 100 |                 // Clear transmitBuf so we know its been sent          
; 101 |                 memset(transmitBuf, 0, RPMSG_MESSAGE_SIZE);            
; 104 |     } else if (uartGetC(receiveBuf)) { // Is there anything to receive?
; 105 |         uint16_t recvLen = receiveRemainingMessage(&receiveBuf[1]);    
; 106 |         pru_rpmsg_send(&transport, dst, src, receiveBuf, recvLen+1);   
; 107 |         memset(receiveBuf, 0, MAX_PAYLOAD_LEN);                        
; 111 |                                                                        
;----------------------------------------------------------------------
        ADD       r14, r2, 0            ; [ALU_PRU] |79| transport,transport
        ADD       r15, r2, 60           ; [ALU_PRU] |79| src,src
        ADD       r16, r2, 62           ; [ALU_PRU] |79| dst,dst
        LDI       r17, ||transmitBuf||  ; [ALU_PRU] |79| transmitBuf
        ADD       r18, r2, 64           ; [ALU_PRU] |79| len,len
        JAL       r3.w2, ||pru_rpmsg_receive|| ; [ALU_PRU] |79| pru_rpmsg_receive
        QBNE      ||$C$L21||, r14.w0, 0x00 ; [ALU_PRU] |79| 
;* --------------------------------------------------------------------------*
...
;* --------------------------------------------------------------------------*
||$C$L23||:    
        ADD       r14, r17, 0x01        ; [ALU_PRU] |94| $O$C3
        JAL       r3.w2, ||receiveRemainingMessage|| ; [ALU_PRU] |94| receiveRemainingMessage
        ADD       r18.w0, r14.w0, 0x01  ; [ALU_PRU] |95| recvLen
        LBBO      &r0.w0, r2, 62, 2     ; [ALU_PRU] |95| dst
        ADD       r14, r2, 0            ; [ALU_PRU] |95| transport,transport
        MOV       r15, r0.w0            ; [ALU_PRU] |95| 
        LBBO      &r0.w0, r2, 60, 2     ; [ALU_PRU] |95| src
        MOV       r16, r0.w0            ; [ALU_PRU] |95| 
        JAL       r3.w2, ||pru_rpmsg_send|| ; [ALU_PRU] |95| pru_rpmsg_send
;* --------------------------------------------------------------------------*
;*   BEGIN LOOP ||$C$L24||
;* --------------------------------------------------------------------------*
||$C$L24||:    
        LDI       r4, ||transmitBuf||   ; [ALU_PRU] |84| $O$C6,transmitBuf
        LBBO      &r0.b0, r4, 0, 1      ; [ALU_PRU] |84| $O$C6
        QBNE      ||$C$L26||, r0.b0, 0x00 ; [ALU_PRU] |84| 
;* --------------------------------------------------------------------------*
        ADD       r14, r2, 0            ; [ALU_PRU] |84| transport,transport
        ADD       r15, r2, 60           ; [ALU_PRU] |84| src,src
        ADD       r16, r2, 62           ; [ALU_PRU] |84| dst,dst
        MOV       r17, r4               ; [ALU_PRU] |84| $O$C6
        ADD       r18, r2, 64           ; [ALU_PRU] |84| len,len
        JAL       r3.w2, ||pru_rpmsg_receive|| ; [ALU_PRU] |84| pru_rpmsg_receive
        QBEQ      ||$C$L26||, r14.w0, 0x00 ; [ALU_PRU] |84| 
;* --------------------------------------------------------------------------*
        LDI       r14, ||receiveBuf||   ; [ALU_PRU] |104| receiveBuf
        JAL       r3.w2, ||uartGetC||   ; [ALU_PRU] |104| uartGetC
        QBEQ      ||$C$L24||, r14.b0, 0x00 ; [ALU_PRU] |104| 
;* --------------------------------------------------------------------------*
        LDI       r4, ||receiveBuf||    ; [ALU_PRU] |105| $O$C5,receiveBuf
        ADD       r14, r4, 0x01         ; [ALU_PRU] |105| $O$C5
        JAL       r3.w2, ||receiveRemainingMessage|| ; [ALU_PRU] |105| receiveRemainingMessage
        MOV       r17, r4               ; [ALU_PRU] |106| $O$C5
        ADD       r18.w0, r14.w0, 0x01  ; [ALU_PRU] |106| recvLen
        LBBO      &r0.w0, r2, 62, 2     ; [ALU_PRU] |106| dst
        ADD       r14, r2, 0            ; [ALU_PRU] |106| transport,transport
        MOV       r15, r0.w0            ; [ALU_PRU] |106| 
        LBBO      &r0.w0, r2, 60, 2     ; [ALU_PRU] |106| src
        MOV       r16, r0.w0            ; [ALU_PRU] |106| 
        JAL       r3.w2, ||pru_rpmsg_send|| ; [ALU_PRU] |106| pru_rpmsg_send
        LDI       r0, 0x00ff            ; [ALU_PRU] |411| length
;* --------------------------------------------------------------------------*
;*   BEGIN LOOP ||$C$L25||
;*
...
$1:     SUB      r1, r1, 1
        QBNE     $1, r1, 0             ; [ALU_PRU] |42| 
        ADD       r0, r0, 0x01          ; [ALU_PRU] |36| i,i
        LDI32     r14, 0x80000064       ; [ALU_PRU] |36| 
        MOV       r1, r0                ; [ALU_PRU] |36| i
        XOR       r1.b3, r1.b3, 0x80    ; [ALU_PRU] |36| 
        QBLT      ||$C$L27||, r14, r1   ; [ALU_PRU] |36| 
;* --------------------------------------------------------------------------*
        MOV       r14, r4               ; [ALU_PRU] |88| 
        LDI       r15.w0, 0x0001        ; [ALU_PRU] |88| 
        JAL       r3.w2, ||uartWrite||  ; [ALU_PRU] |88| uartWrite
        LDI       r17, ||receiveBuf||   ; [ALU_PRU] |92| $O$C3,receiveBuf
        MOV       r14, r17              ; [ALU_PRU] |92| $O$C3
        JAL       r3.w2, ||uartGetC||   ; [ALU_PRU] |92| uartGetC
        QBEQ      ||$C$L28||, r14.b0, 0x00 ; [ALU_PRU] |92| 
;* --------------------------------------------------------------------------*
        LDI32     r0, 0x80000000        ; [ALU_PRU] |92| 
        LBBO      &r1.b0, r17, 0, 1     ; [ALU_PRU] |92| $O$C3
        LBBO      &r14.b0, r4, 0, 1     ; [ALU_PRU] |92| $O$C4
        XOR       r1, r1.b0, r0         ; [ALU_PRU] |92| 
        XOR       r0, r14.b0, r0        ; [ALU_PRU] |92| 
        QBGT      ||$C$L23||, r1, r0    ; [ALU_PRU] |92| 
;* --------------------------------------------------------------------------*
||$C$L28||:    
        LBBO      &r0.w0, r2, 64, 2     ; [ALU_PRU] |99| len
        ADD       r14, r4, 0x01         ; [ALU_PRU] |99| $O$C2
        SUB       r15.w0, r0.w0, 0x01   ; [ALU_PRU] |99| 
        JAL       r3.w2, ||uartWrite||  ; [ALU_PRU] |99| uartWrite
